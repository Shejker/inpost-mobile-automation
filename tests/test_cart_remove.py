    ...
    def test_remove_all_items_from_cart(self, driver):
        ...
        while True:
            ...
            for i in range(len(add_to_cart_buttons)):
                try:
                    buttons = driver.find_elements(AppiumBy.XPATH, '//android.widget.TextView[@text="ADD TO CART"]')
                    if i < len(buttons):
                        buttons[i].click()
                        products_added += 1
                        take_screenshot(driver, self.screenshots_dir, f"01_product_{products_added}_added")
        ...
        while removed_count < products_added:
            ...
            assert not cart_page.item_exists(item_name), f"Item '{item_name}' still in cart"
