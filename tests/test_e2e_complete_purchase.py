...
class TestCompletePurchaseFlow:
    def test_complete_purchase_with_autofill_login(self, driver):
        ...
        try:
            ...
            cart_icon = products_page.find_element(
            ...
            logger.info(
                f"  [OK] Product {product['name']} ({product['price']}) verified in cart"
            )
            ...
            checkout_overview_page.verify_product_in_overview(
                product["name"], product["price"]
            )
            logger.info(
                f"  [OK] Product '{product['name']}' ({product['price']}) confirmed in overview"
            )
            ...
            logger.info("=" * 80)
            logger.info(
                "TEST COMPLETE: Purchase finished successfully and returned home"
            )
            logger.info("=" * 80)
