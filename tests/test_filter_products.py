...
class TestFilterProducts:
    ...
    def _collect_products(self, driver, max_iterations=5):
        ...
        while iteration < max_iterations:
            iteration += 1
            add_to_cart_buttons = driver.find_elements(AppiumBy.XPATH, '//android.widget.TextView[@text="ADD TO CART"]')
            
            if not add_to_cart_buttons:
                logger.info("  [OK] No more products found")
                break
            
            items = driver.find_elements(AppiumBy.XPATH, '//android.view.ViewGroup[@content-desc="test-Item"]')
            for item in items:
                try:
                    name_element = item.find_element(AppiumBy.XPATH, './/android.widget.TextView[@content-desc="test-Item title"]')
                    price_element = item.find_element(AppiumBy.XPATH, './/android.widget.TextView[@content-desc="test-Price"]')
    ...
    @pytest.mark.parametrize("filter_name,sort_key,reverse", [
        ("Name (A to Z)", lambda x: x['name'], False),
        ("Name (Z to A)", lambda x: x['name'], True),
        ("Price (low to high)", lambda x: float(x['price'].replace('$', '').strip()), False),
        ("Price (high to low)", lambda x: float(x['price'].replace('$', '').strip()), True),
    ])
    def test_filter_products(self, driver, filter_name, sort_key, reverse):
        ...
        if isinstance(product_keys[0], float):
            for i, (actual, expected) in enumerate(zip(product_keys, sorted_keys)):
                assert abs(actual - expected) < 0.01, f"Product {i+1} price mismatch: {actual} != {expected}"
        else:
            assert product_keys == sorted_keys, f"Products not sorted correctly. Got: {product_keys}, Expected: {sorted_keys}"
